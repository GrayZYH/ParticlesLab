		...
		protected void GenerateFilename()
		{
			if (_outputType == OutputType.VideoFile)
			{
				if (_autoGenerateFilename || string.IsNullOrEmpty(_forceFilename))
				{
                    ...
                    _VideoName = GenerateTimestampedFilename(_autoFilenamePrefix, _autoFilenameExtension);
                    _filePath = AutoGenerateFilename(_outputFolderType, _outputFolderPath, _VideoName);
				}
				else
				{
